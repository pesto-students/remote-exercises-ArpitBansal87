function timeWas(time, now) {
  if (
    typeof time !== 'number'
    || (arguments.length === 2 && typeof now !== 'number')
  ) {
    throw new TypeError();
  }
  ...
  if (timeDifference / SECONDS < 60) {
    return `${timeDifference / SECONDS} second${
      timeDifference / SECONDS !== 1 ? 's' : ''
    } ago`;
  }
  if (timeDifference / MINUTES < 60) {
    return `${timeDifference / MINUTES} minute${
  ...
  if (timeDifference / WEEKS < 5) {
    return `${timeDifference / WEEKS} week${
      timeDifference / WEEKS !== 1 ? 's' : ''
    } ago`;
  }
  if (timeDifference / MONTHS < 12) {
    return `${timeDifference / MONTHS} month${
      timeDifference / MONTHS !== 1 ? 's' : ''
    } ago`;
  }
  return `${timeDifference / YEARS} year${
